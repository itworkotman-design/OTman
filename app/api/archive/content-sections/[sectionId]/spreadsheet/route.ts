...
import { getOrCreateSpreadsheet, updateSpreadsheet } from "@/lib/docArchive/spreadsheets";
...
export async function GET(
  ...
  const data = await getOrCreateSpreadsheet(ctx.companyId, ctx.tenantId, section.itemId, sectionId);
...
export async function PATCH(
  ...
  const updateResult = await updateSpreadsheet(ctx.companyId, ctx.tenantId, section.itemId, sectionId, body?.data);
  if (!updateResult.ok) {
