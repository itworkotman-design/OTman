...
import { getOrCreateTitle, updateTitle } from "@/lib/docArchive/titles";
...
export async function GET(
  ...
  const text = await getOrCreateTitle(ctx.companyId, ctx.tenantId, section.itemId, sectionId);
...
export async function PATCH(
  ...
  const updateResult = await updateTitle(ctx.companyId, ctx.tenantId, section.itemId, sectionId, body?.text);
  if (!updateResult.ok) {
