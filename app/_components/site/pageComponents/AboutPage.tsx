  ...
  return (
    <div className="">
      ...
      <section className="pt-[400]">
        <p className="my-10 text-xl whitespace-pre-wrap">{content.introText[locale]}</p>
      </section>
      <section>
        <div className=" my-20">
        ...
        <div className="mx-auto px-6 my-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 justify-items-center">
            {content.teamMembers.map((member) => (
              <PersonItem key={member.id} src={member.src} name={member.name} position={member.position[locale]} email={member.email} />
            ))}
          </div>
        </div>
      </section>
