...
import type {
  ...
  ArchiveContextInput,
  ArchiveEffectiveCapabilitySource,
  ArchivePermission,
  ArchivePermissionAction,
  ArchivePermissionEffect,
  ArchivePermissionExplanation,
  ArchivePermissionExplanationDecidingTarget,
  ArchivePermissionSubjectType,
  ArchivePermissionTargetType,
  ArchiveRoleAssignmentSummary,
  ArchiveRoleSummary,
} from "@customprojects/custom-archive";

// OTman host note: these two constants are genuine runtime VALUES on the
// real package (`@customprojects/custom-archive`'s permission-types module),
// but importing them as values from the package ROOT (`./bridge`'s only
// sanctioned import path) drags the package's Prisma-backed implementation
// — and therefore `@prisma/client`'s Node-only runtime (`node:module`) —
// into this file's module graph. That's fine for a server-only consumer, but
// this screen is mounted client-side, and Turbopack/webpack correctly refuse
// to bundle a Node-only module for the browser. Re-declared locally,
// byte-identical to the package's own values (confirmed against
// node_modules/@customprojects/custom-archive/dist/archive/permissions/permission-types.js),
// so this file never imports a *value* from the package root.
const ARCHIVE_PERMISSION_ACTIONS: readonly ArchivePermissionAction[] = [
  "view",
  "create",
  "upload",
  "edit",
...
const ARCHIVE_NAMESPACE_PERMISSION_ACTIONS: readonly ArchivePermissionAction[] = [
  "create",
  "manage_permissions",
];
