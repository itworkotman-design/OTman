...
Provides the shared create and resolve helpers that keep `OrderNotification` rows and the derived unread-notification counters on `Order` in sync. Alert rules live in `lib/orders/alerts` and use these helpers to light the archive `Alerts` cell until the notification is marked fixed.
