...
Builds and parses order conversation email metadata, including internal thread tokens, reply aliases, subjects, sender addresses, and the HTML/plain-text bodies used by order conversation messages. Customer-visible subject and body output do not include `[OTMAN:<threadToken>]`; routing uses `reply+<threadToken>@reply.otman.no` and stored Gmail thread metadata. Conversation signatures use the configured Gmail send-as alias.
...
| `buildOrderConversationEmailText` | Builds the plain-text order conversation email body with optional quoted context and no visible token marker. |
| `buildOrderConversationEmailHtml` | Builds the HTML order conversation email body with optional quoted context and no visible token marker. |
