...
Builds normalized order snapshots and change descriptions for order history entries.
...
| `buildOrderEventSnapshot` | Creates a normalized order snapshot used for history diffs. It now includes the custom-time `Contact customer?` flag and optional contact note. |
