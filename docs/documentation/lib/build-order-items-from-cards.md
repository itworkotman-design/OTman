...
Converts saved product cards into persisted order item rows, including option metadata, visible custom-section selections, read-only WordPress mismatch rows with saved quantities, and the customer/subcontractor prices used for the order.
...
| `buildOrderItemsFromCards` | Builds product-card, option, visible custom-section, demont, return, and read-only WordPress mismatch item rows using effective customer prices for discounts, preserving read-only WP quantity and unit-price metadata. |
