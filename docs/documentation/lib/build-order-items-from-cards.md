...
Converts saved product cards into persisted order item rows, including option metadata and the customer/subcontractor prices used for the order.
...
| `buildOrderItemsFromCards` | Builds product-card, option, custom-section, demont, and return item rows using effective customer prices for discounts. |
