...
Imports legacy WordPress `power_order` posts into the new `Order` table. The route authenticates the sync secret, maps the legacy WordPress author to an active membership, maps the selected WordPress subcontractor onto an active subcontractor membership when it matches an existing user, stores `displayId` from the WordPress post id instead of consuming the manual app order counter, preserves imported created and modified timestamps, resolves contact fields through multiple legacy key aliases, normalizes legacy delivery dates, time windows, lift values, statuses, and Norwegian deviation values into the editor's native formats, overrides the legacy time-window dropdown with `endre_tid_fra` / `endre_tid_til` when those fields contain real non-midnight times so the editor opens the order as custom time, reads status from `status`, `order_status`, or `post_status`, normalizes legacy extra pickup addresses and default contact rows from both nested repeater data and flattened ACF meta keys, preserves legacy express-delivery state from the saved checkbox, from `price_breakdown_html`, or from parsed express service rows, parses legacy product and service rows from any WordPress repeater prefix that ends in `_velg_produkt`, reads install and return selections from both structured repeater labels and checkbox-style truthy legacy fields including fixed ACF field ids such as the old return selector, deduplicates services that appear in both structured meta and `price_breakdown_html`, preserves the WordPress return option as selected even when a separate return address exists, compares each WP product group total against the mapped native product total, keeps exact matches editable, stores mismatches as gray read-only WordPress price snapshots, includes per-card WordPress subcontractor totals inside those snapshots when the subcontractor breakdown is available, preserves WP KM and extra-pickup price rows as read-only snapshots, imports customer discounts, subcontractor minus adjustments, price-list-driven deviation fees, and hardcoded order fee fields while ignoring manual plus extras, rebuilds native `OrderItem` rows, keeps the original WordPress labels in `rawData`, stores imported distance and price totals as the final order totals when present, syncs the WordPress price-mismatch alert against that effective stored total, and copies WordPress attachments into local app storage while preserving their original source URLs. Historical script-only debug logging has been removed from this route.
...
| `normalizeImportedManualTime` | Normalizes legacy `endre tid` values such as `12am`, `12:30`, and `12:30:00` into `HH:mm` values. |
| `getImportedManualTimeWindow` | Builds a custom time-window override from `endre_tid_fra` and `endre_tid_til`, ignoring the old app's midnight placeholder. |
| `getImportedTimeWindow` | Chooses the manual `endre tid` override when present, otherwise falls back to the legacy `tidsvindu_for_levering` dropdown value. |
...
| `buildParsedServiceCoverageKey` | Creates a stable service identity key so structured meta rows are skipped when the same service was already parsed from `price_breakdown_html`, even if the two sources disagree on row price or quantity. |
...
| `parsePriceSettingCents` | Converts a price-list settings price string into integer cents. |
| `getDeviationPriceCents` | Resolves price-list customer and subcontractor cents for a deviation option. |
...
| `getNativeCalculatedPricing` | Rebuilds the native ex-VAT and subcontractor totals from mapped product cards, the current booking catalog, imported admin adjustment fields, price-list-driven deviation fees, and imported hardcoded fee fields. |
...
| `syncWordpressPriceMismatchAlert` | Creates or resolves the WordPress manual-review alert for the effective comparison total passed by the importer; when a WordPress total is persisted as the final order total, the route passes that stored total as the native comparison value. |
