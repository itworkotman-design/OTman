...
| `PATCH` | Updates the order, validates and normalizes extra-pickup contacts, applies saved pricing snapshots with the selected price list's global settings before rebuilding summaries/items from product cards, persists native manual adjustments and hardcoded fee fields, writes order events, creates a fresh extra-pickup order notification when the extra-pickup list changes, preserves the normalized product-card snapshot, and saves the custom-time `Contact customer?` flag plus its optional contact note. It also persists edited return addresses. |
