...
Reads and updates a single order, including product-card snapshots, event snapshots, order-item rebuilding, and extra-pickup alert creation when extra pickups change. The read path now rebuilds missing legacy WordPress extra pickups from `legacyWordpressRawMeta` when older imports never populated the structured extra-pickup fields.
...
| `PATCH` | Updates the order, validates and normalizes extra-pickup contacts, rebuilds summaries/items from product cards, writes order events, creates a fresh extra-pickup order notification when the extra-pickup list changes, preserves the normalized product-card snapshot, and saves the custom-time `Contact customer?` flag plus its optional contact note. It also persists edited return addresses. |
