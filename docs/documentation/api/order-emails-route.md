...
Loads one order's email conversation, sends admin replies, records successful outbound messages, and records failed outbound attempts with the provider error so they appear inside the Alert Center conversation.

## Functions

| Function | Description |
| --- | --- |
| `getAdminMembership` | Validates that the current request belongs to an active admin or owner in the selected company. |
...
| `POST` | Sends an admin email reply, stores successful outbound messages, and stores failed outbound messages plus the provider error when sending fails. |
