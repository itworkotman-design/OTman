...
| `OrderEmailModal` | Main Alert Center modal. Loads conversation, notifications, and history; shows the renamed alert-center tabs; keeps the customer email/name fields editable, defaults them from the active order recipient instead of the first stored outbound message, lets admins send email replies, refreshes the conversation after failed sends so stored `FAILED` messages appear immediately, lets admins mark email conversations complete, and lets admins resolve order notifications so yellow archive alerts clear. |
