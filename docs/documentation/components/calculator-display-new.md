...
| `CalculatorDisplayNew` | Main calculator display component. It renders a saved product-card `modelNumber` in muted gray parentheses beside the product heading when one exists, shows read-only WordPress mismatch groups in gray, shows applied customer discount and extra rows above the total, exposes localized manual adjustment inputs for native admin edits, and leaves negative totals visible instead of hiding them. |
