...
Owns the archive filter state, renders the status/store/subcontractor/date/search controls, optionally renders a selected-table download button, keeps status option values aligned with the canonical lowercase order-status keys, auto-applies filter changes with a short debounce, raises quick and custom date ranges to the 1000-row show-all page size, shows the loaded row count as placeholder text for those show-all date ranges instead of displaying `1000` in the orders-per-page input, clears active date filters when a page-size shortcut button is clicked, keeps the requested orders-per-page value stable while results load, syncs the latest parent `onApply` callback into a ref through an effect so parent rerenders do not trigger repeated fetch loops, skips duplicate apply calls on first render and reset, and resets the selected filter values without needing an explicit Apply button.
