...
Renders the upload UI for order files, split into two user-facing categories: receipts and attachments. The section localizes its built-in labels from `locale` and uses separate open and download URLs so S3-backed PDFs and images can open from signed URLs while downloads still use the explicit download target.
