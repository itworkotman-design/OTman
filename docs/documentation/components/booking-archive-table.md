...
Renders the archive table for admin, subcontractor, and order-creator views using the active column configuration. The admin table now shows the assigned membership `Store` before `Customer name`, the admin customer column reads from the order's `customerName` field instead of store data, subcontractor rows show only the subcontractor price, the status badge now uses the shared status-normalization helper for both its visible label and hover title, archive dates render in explicit `dd/mm/yyyy` or `dd/mm/yyyy HH:mm` format instead of browser-dependent locale output, and the outer scroll container now expands to the real remaining page width instead of keeping a stale desktop max-width when the sidebar collapses.
